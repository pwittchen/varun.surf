# Batch 5 — Netherlands (4 spots)

Run in a fresh context. Use the **`kite-spot-creator`** agent per spot.
`country` = `"Netherlands"`. Classic Dutch spots — strong, reliable wind,
shallow/flat options. Tide matters on the North Sea / delta spots.
...
- See issue link `https://kitesurfvereniging.nl/spotkaart/` for exact launches;
  extra NL spots from that map are deferred to **batch 10 (research)**.
- Windfinder slugs: `ijmuiden`, `brouwersdam`, `slufter` etc. — verify each.
- Water cold (North Sea ~6–19°C); season broadly spring–autumn, windy all year.
